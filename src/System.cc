...
vector<cv::Mat> System::GetTrajectoryTUM()
{
  ...
  /*
    ...
    for(list<cv::Mat>::iterator lit=mpTracker->mlRelativeFramePoses.begin(),
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++, iter_nm++)
    {
        if(*lbL)
            continue;
...
*/
 for (list<Tracking::TrackedFrame>::iterator iter = mpTracker->tracked_frames.begin(), iter_end = mpTracker->tracked_frames.end(); iter != iter_end; iter++)
 {
	...
	cv::Mat row = (cv::Mat_<float>(1,8,CV_32F) << iter->time, //float(*lT), 
						twc.at<float>(0), 
						twc.at<float>(1),
						twc.at<float>(2),
						q[0],q[1],q[2],q[3]);
...
vector<cv::Mat> System::GetKeyFrameTrajectoryTUM()
{
	...
	cv::Mat row = (cv::Mat_<float>(1,8,CV_32F) << pKF->mTimeStamp,
					t.at<float>(0),
					t.at<float>(1),
					t.at<float>(2),
					q[0],q[1],q[2],q[3]);
...
cv::Mat System::GetWorldPose() {
    ...
    if(Tcw.empty()) {
        ORB_SLAM2::KeyFrame* pKF = mpTracker->tracked_frames.back().reference_keyframe;//mpTracker->mlpReferences.back();
        while(pKF->isBad()) {
            Trw = Trw * pKF->mTcp;
            pKF = pKF->GetParent();
        }
        Trw = Trw * pKF->GetPose();
        cv::Mat relPose = mpTracker->tracked_frames.back().relative_frame_pose;//mpTracker->mlRelativeFramePoses.back();
        Tcw = relPose * Trw;
    }
